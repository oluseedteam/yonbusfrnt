            ...
            <div class="card-box">
                ...
                @if($tr->refund_amount || $tr->tax_due)
                    <div class="mt-6 pt-4 border-t border-gray-100 dark:border-gray-800 grid grid-cols-2 gap-4 max-w-xs text-xs">
                        @if($tr->refund_amount)
                            <div class="p-3 rounded-xl bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-100 dark:border-emerald-900/50">
                                <span class="text-emerald-700 dark:text-emerald-300 font-semibold block text-[11px]">Estimated Refund</span>
                                <span class="text-lg font-extrabold text-emerald-600 font-heading">${{ number_format($tr->refund_amount, 2) }}</span>
                            </div>
                        @endif
                        @if($tr->tax_due)
