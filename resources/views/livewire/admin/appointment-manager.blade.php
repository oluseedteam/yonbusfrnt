<div class="p-6">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
            ...
            <p class="text-slate-600 dark:text-slate-400 text-sm">Monitor, confirm, complete, and cancel client appointments across the platform</p>
        </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="border-b border-slate-200 dark:border-slate-700 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider bg-slate-50 dark:bg-slate-900/50">
                        ...
                        <th class="px-6 py-4">Accountant</th>
                ...
                <tbody class="divide-y divide-slate-200 dark:divide-slate-700 text-sm text-slate-700 dark:text-slate-300">
                    @forelse($appointments as $appt)
                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition">
                            ...
                            <td class="px-6 py-4 text-slate-600 dark:text-slate-400">{{ $appt->accountant?->name ?? 'Unassigned' }}</td>
                            <td class="px-6 py-4 font-medium text-slate-900 dark:text-white">{{ $appt->service?->name ?? 'N/A' }}</td>
                            ...
                            <td class="px-6 py-4">
                                <span class="inline-flex px-2.5 py-1 text-xs font-semibold rounded-full 
                                    ...
                                    {{ $appt->status === 'confirmed' ? 'bg-blue-500/10 text-blue-600 dark:text-blue-400' : '' }}
                                    {{ $appt->status === 'completed' ? 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400' : '' }}
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-6 py-12 text-center text-slate-500 dark:text-slate-400">
                                ...
                                <p class="text-xs">Appointments will appear here when booked by clients or scheduled by accountants.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
